...
public class SelectPage extends BasePage
{
  ...
  public SelectPage(PageParameters parameters)
  {
    ...
    add(new Code("default-html-code", Model.of(formHtmlCode)).setShowLineNumbers(true), new Code(
        "default-java-code", Model.of(formJavaCode)).setShowLineNumbers(true));
  ...
  private Fragment fragment(String wicketId, boolean i18n)
  {
    ...
    fr.add(first, newSelect("default-live-search", of(i18n).withLiveSearch(true)));
    fr.add(
        newMultiSelect("multi", of(i18n)), newMultiSelect("multi-live-search", of(i18n).withLiveSearch(true)),
  ...
  private BootstrapSelectConfig of(boolean i18n)
  {
    ...
    if (i18n)
    {
      ...
      config
          .withNoneSelectedText("My nothing selected").withNoResultText("My no results found")
          .withCountSelectedText("My selected {0} from {1}")
          .withMaxOptionsText("My limit ({n} {var} max)", "My group limit({n} {var} max)", "items", "item");
    }
    return config;
  }
